# app.py — COMPLETE BATCH TRANSCRIBER (FIXED FOR INCOMPLETE TRANSCRIPTS)
# -----------------------------------------------------------------------------
# FEATURES INCLUDED:
...
# 7. High Concurrency (Slider up to 128 workers).
# 8. [NEW] Download Integrity Check (Prevents partial file uploads).
# 9. [NEW] Increased Timeout (Prevents cut-offs on long calls).
# -----------------------------------------------------------------------------
...
# NOTE: Using "gemini-3-flash-preview" for speed and stability.
...
def generate_transcript(api_key: str, file_uri: str, mime_type: str, prompt: str) -> str:
    ...
    for attempt in range(max_content_retries):
        # INCREASED TIMEOUT TO 600s (10 MINS) TO PREVENT TIMEOUTS ON LONG CALLS
        resp = make_request_with_retry("POST", api_url, json=payload, headers={"Content-Type": "application/json"}, timeout=600)
...
1. This is a conversation between an AGENT (service provider) and a CUSTOMER.
2. You MUST identify who is the Agent and who is the Customer based on the context (e.g., person offering help is the Agent).
3. Use the labels 'Agent:' and 'Customer:' instead of 'Speaker 1' and 'Speaker 2'.
4. MAINTAIN CONSISTENCY: The Agent should not suddenly become the Customer.
5. If you absolutely cannot distinguish roles, use 'Speaker 1' and 'Speaker 2', but prioritize 'Agent' and 'Customer'.

FORMATTING REQUIREMENTS:
- EVERY line must start with a timestamp and label.
- Format: [0ms-1500ms] Label: Dialogue
- Timestamps must be in raw milliseconds.
- NEVER merge dialogue from two speakers in one line.
...
EXAMPLE:
[0ms-2500ms] Agent: Hello, welcome to customer support.
[2600ms-4000ms] Customer: Hi, I am calling to check my balance.

Transcribe the ENTIRE audio file from start to finish. Do NOT output unlabeled lines.
...
def process_single_row(index: int, row: pd.Series, api_key: str, prompt_template: str, keep_remote: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        # [NEW] Check Expected Size (if available)
        ...
        # [NEW] Integrity Check: Verify we got the whole file
        if expected_size:
            expected_size_int = int(expected_size)
            if file_size < expected_size_int:
                raise Exception(f"Incomplete download: Expected {expected_size_int} bytes, got {file_size}")
